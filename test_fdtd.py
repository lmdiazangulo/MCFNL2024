...
class FDTD1D():
    ...
    def step(self):
        ...
        E[1:-1] = E[1:-1] *  (c_eps2[1:-1] / c_eps3[1:-1]) - c_eps[1:-1] * (H[1:] - H[:-1])
...
def test_energy_conservation():
    ...
    mu = 1
    epsilon_vector = epsilon_r * np.ones(x.size)
    conductivity_vector = conductivity * np.zeros(x.size)
    ...
    fdtd.run_until(dx)
    E_final = fdtd.getE()
    final_energy = energy(E_final, H_final) 

    assert np.isclose(initial_energy, final_energy, atol=1e-2)
    
def test_pml():
    x = np.linspace(-2.0, 2.0, num=421)
    inter = 401
    pml_size = 10
    dx = np.abs(x[1] - x[2])
